...
const Citas = () => {
  ...
  const cargarCitas = async () => {
    try {
      ...
      // NOTA: Se eliminó el envío de 'especialidad' al backend para evitar 
      // que el servidor devuelva vacío si no soporta este parámetro. 
  ...
  // Filtrado en el frontend para garantizar que funcione inmediatamente
  const citasFiltradas = useMemo(() => {
    if (!filtroEspecialidad) return citas;
    
    return citas.filter((cita) => {
      const espCita = (cita.especialidad_medico || "").trim().toLowerCase();
      const espFiltro = filtroEspecialidad.trim().toLowerCase();
      return espCita === espFiltro;
    });
  }, [citas, filtroEspecialidad]);
      ...
      <html>
        ...
        <body>
          <div class="header">
            ...
            <p class="date">Total de citas mostradas: ${citasFiltradas.length}</p>
            ${filtroEspecialidad ? `<p class="date">Filtro aplicado: Especialidad - ${filtroEspecialidad}</p>` : ''}
          </div>
          <table>
            <thead>
              <tr>
  ...
  // Extraer especialidades únicas de los médicos cargados para el selector
