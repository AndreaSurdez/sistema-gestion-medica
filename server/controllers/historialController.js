...
exports.registrarEntrada = async (req, res) => {
  const { paciente_id, cita_id, motivo_consulta, diagnostico, tratamiento, observaciones } = req.body;
  ...
  try {
    ...
    if (!medico_registro) {
      return res.status(403).json({ error: 'Solo los médicos pueden registrar historial clínico' });
    }
    ...
    const result = await pool.query(
      `INSERT INTO historial_clinico 
       (paciente_id, cita_id, motivo_consulta, diagnostico, tratamiento, observaciones, medico_registro)
    ...
    if (cita_id) {
      await pool.query(
        "UPDATE citas SET estado = 'finalizada' WHERE id = $1",
        [cita_id]
      );
    }
    ...
    logger.info(`Historial registrado: Paciente ${paciente_id} por médico ${medico_registro}`);
